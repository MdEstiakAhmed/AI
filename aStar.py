...
def getMin(children):
    ...
    while (l > 0):
        l = l - 1
        if (children[l]['value'] < min):
            min = children[l]['value']
            minChild = children[l]
    return minChild;

def getSum(current, sld, path):
    total = 0
    for index in range(len(path)):
        total = total + path[index]
    total = total + current + sld
    return total
    ...
    while (True):
        if (current != end):
            ...
            for index in range(len(expandAreaList)):
                expandAreaValueList.append({
                    "area": expandAreaList[index],
                    "value": getSum(roadMap[current][expandAreaList[index]], SLD[expandAreaList[index]]['distance'], pathValueList),
                    "pathCost": roadMap[current][expandAreaList[index]]
                })
